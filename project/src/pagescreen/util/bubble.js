...
export default (function(win, doc) { // ns
    // 判断对象
    ...
    function TagCloud (options) {
       ...
       // self.box.style.minWidth = 2.5 * self.size + "px";
        for (var i = 0;  i < self.items.length; i++) {
            self.items[i].element.style.position = "absolute";
            self.items[i].element.style.zIndex = i + 1;
        }
    ...
    TagCloud._getConfig = function (config) {
        ...
        if(isObject(config)) {
            for(var i in config) {
                if(Object.prototype.hasOwnProperty.call(config, i)) {//hasOwnProperty()用来判断一个属性是定义在对象本身而不是继承自原型链
